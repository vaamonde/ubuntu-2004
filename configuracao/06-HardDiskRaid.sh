...
# Kernel >= 5.14.x
#
# RAID foi originalmente denominado de "Redundant Array of Inexpensive Drives" 
# (Conjunto Redundante de Discos Baratos). Com o tempo, numa tentativa de dissociar 
# o conceito de "discos baratos", a indústria reviu o acrônimo para "Redundant Array 
# of Independent Disks" (Conjunto Redundante de Discos Independentes). RAID é um meio 
# de se criar um subsistema de armazenamento composto por vários discos individuais, 
# com a finalidade de ganhar segurança -- por meio da redundância de dados e desempenho.
#
# Tipos de RAID (Obs: os RAID 0 e 1 obrigatoriamente precisa de 2 (dois) Hard Disk, os 
# RAID superiores ao 5 precisam de 3 (três) ou mais Hard Disk para serem configurados)
...
#
# Fdisk: é um programa orientado a menus para criação e manipulação de tabelas de partição, 
# ele compreende tabelas de partição do tipo DOS e etiquetas de disco do tipo BSD ou SUN. 
# O fdisk não entende GPTs (tabelas de partição GUID) e não foi projetado para partições 
# grandes, nesses casos, use o GNU mais avançado dividido. O fdisk não usa o modo e os 
# cilindros compatíveis com DOS como unidades de exibição por padrão.
#
# Cfdisk: é um editor de partições do Linux, semelhante ao fdisk, mas com uma interface 
# de usuário diferente (ncurses). Faz parte do pacote de programas utilitários do Linux, 
# o util-linux. 
#
# Parted: (o nome sendo a conjunção das duas palavras PARTition e EDitor) é um editor 
# de partições livre, usado para criar e excluir partições. Isso é útil para criar 
# espaço para novos sistemas operacionais, reorganizar o uso do disco rígido, copiar 
# dados entre discos rígidos e imagens de disco.
#
# Mdadm: é um utilitário Linux usado para gerenciar e monitorar dispositivos RAID de 
# software, é usado em distribuições Linux modernas no lugar de utilitários RAID de 
# software mais antigos, como raidtools2 ou raidtools.
#
# Mkfs: é um comando usado para formatar um dispositivo de armazenamento em bloco com 
# um sistema de arquivos específico. O comando é parte dos sistemas operacionais UNIX 
# e tipo UNIX.
#
# Mount ou Umount: tem a função de montar ou desmontar um dispositivo na hierarquia do 
# sistema de arquivos do Linux, muito utilizado para acessar partições em Hard Disk, 
# Pendrive, CD-Rom, DVD, etc... 
#
# Fstab: O arquivo /etc/fstab permite que as partições do sistema sejam montadas 
# facilmente especificando somente o dispositivo ou o ponto de montagem. Este arquivo 
# contém parâmetros sobre as partições que são lidos pelo comando mount. 
#
